...
class Bilinear(nn.Module):
    ...
    Ordinarily bilinear models allow for the left and right components
    to have different number of features, but as with our pairwise
    models, we will not support different numbers of features.
    """
    ...
    def __init__(self,
        ...
        Args:
            in_features (int): Number of features in left/right components.
            out_features (int): Number of output features.
            project (Optional[projections.PairwiseProjection], optional): Apply
                this transformation to inputs first.
                By default, no transformation.

        Raises:
            ValueError: If projection outputs tensors of shape incompatible
                with this model.
        ...
        if project is not None:
            # This models need not support shape coercion, so enforce
        ...
        self.project = project
        self.compat = nn.Bilinear(in_features, in_features, out_features)
...
class BiMLP(nn.Module):
    """MLP that computes compatibilities between representations pairs.

    Given two tensors of shape (*, in_features), optionally projects
    both representations down with a PairwiseProjection, and then concatenates
    the tensors on their last dimension and feeds them to an MLP (see above).
    """
    ...
    def __init__(self,
        ...
        if project is not None:
            # This model need not support shape coercion, so enforce
            # output features are what the bilinear module expects.
            if project.out_features != in_features:
                raise ValueError(f'projection must output {in_features} '
                                 f'features, got {project.out_features}')
    ...
    def forward(self, lefts: torch.Tensor,
        ...
        Args:
            lefts (torch.Tensor): Shape (*, in_features) matrix, where * is any
                number of dimensions.
            rights (torch.Tensor): Same as lefts. Will be concatenated to lefts
